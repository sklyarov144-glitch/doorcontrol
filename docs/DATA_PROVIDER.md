...
Экран `documents` в production хранит метаданные в `document_items`, а бинарные
файлы — в приватном Supabase Storage. Открытие файла выполняется по короткоживущей
подписанной ссылке. Добавление ссылки на акт ОХ также создаёт документ и обновляет
единственную запись `custody_acts` по `door_id`.

Рабочее замечание ТН синхронизируется с `tn_issues` при сохранении двери.
Повторное сохранение обновляет активное замечание, а статус «устранено» закрывает
его с фактическим `resolved_at`. Production-экран замечаний читает этот журнал.
