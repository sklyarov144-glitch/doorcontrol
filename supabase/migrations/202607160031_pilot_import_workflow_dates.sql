      ...
      for floor_data in select value from jsonb_array_elements(building_data -> 'floors') loop
        ...
        for door_data in select value from jsonb_array_elements(floor_data -> 'doors') loop
          ...
          insert into public.doors (
            ...
            assigned_user_id, x, y, model, width_fact, height_fact,
            mounted_at, tn_accepted_at, custody_act_uploaded_at, custody_act_closed_at,
            meta
          ) values (
            ...
            nullif(door_data ->> 'mountedAt', '')::timestamptz,
            nullif(door_data ->> 'tnAcceptedAt', '')::timestamptz,
            nullif(door_data ->> 'custodyActUploadedAt', '')::timestamptz,
            nullif(door_data ->> 'custodyActClosedAt', '')::timestamptz,
          ...
          on conflict (legacy_id) do update set
            ...
            tn_accepted_at = excluded.tn_accepted_at,
            custody_act_uploaded_at = excluded.custody_act_uploaded_at,
            custody_act_closed_at = excluded.custody_act_closed_at,
            meta = excluded.meta;
...
comment on function public.import_pilot_hierarchy(uuid, jsonb) is
  'Transactional, idempotent hierarchy and workflow-date import for trusted pilot tooling only.';
